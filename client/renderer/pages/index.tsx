...
import AppControlHandler, {
	AppControlContext,
} from '../components/appControlHandler';
...
const Home: NextPage = () => {
	...
	const {
		...
		setGlobalInterfaceServer,
		setGlobalProjectManager,
		configureWebSocket,
		initialDMXSetup,
	...
	useEffect(() => {
		...
		if (!app_initialConfigOver) {
			initLoading();
		}
	}, [router.isReady]);

	async function initLoading() {
		setGlobalInterfaceServer((e) => {
			e = new InterfaceServer();
			return e;
		});

		setGlobalProjectManager((e) => {
	...
	// wait for initLoading() to initialize the global variables
	useEffect(() => {
		if (GlobalProjectManager !== null && GlobalInterfaceServer !== null) {
			loadProjects();
		}
	}, [GlobalProjectManager, GlobalInterfaceServer]);
	...
	return (
		<div className="flex justify-center bg-gray-100 dark:bg-gray-900 text-black h-screen w-screen overflow-hidden dark:text-white">
			{error !== null ? (
									...
									<div className="self-center align-middle">
										...
										<ul>
											{availableInterfaces.map((e) => (
												<button
													key={e}
													className="rounded-lg bg-blue-500 p-2 m-2 text-white"
													onClick={(_) => {
